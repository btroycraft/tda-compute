...
bool init_vec(Vec *vec){
  void *init = malloc(INIT_ALLOC_SIZE);
  if(init){
    *vec = (Vec) {.add = NULL, .init = NULL, .size = 0, .alloc = 0};
    return false;
  }
  else{
    *vec = (Vec) {.add = NULL, .init = NULL, .size = 0, .alloc = INIT_ALLOC_SIZE};
    return true;
  }
}
...
bool exp_vec(Vec *vec, size_t num){
  ...
  size_t allocMultInt = (size_t) ALLOC_MULT;
  double allocMultFrac = ALLOC_MULT - allocMultInt;
